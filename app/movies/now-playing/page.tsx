...
import { getNowplayingMovies } from '@/lib/actions/movies.actions';
...
const Page = () => {
  ...
  const loadMovies = async (pageNumber: number) => {
    const res = await getNowplayingMovies(pageNumber);
  ...
  return (
    <main className='flex flex-col items-center justify-center py-10 container mx-auto px-4 sm:px-6 lg:px-8'>
      <h2 className='text-4xl font-semibold text-indigo-700 dark:text-indigo-500 mb-10'>Now-Playing Movies</h2>
